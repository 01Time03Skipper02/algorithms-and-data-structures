...
//Отправляю stackmachine ещё вот в таком виде. Здесь просто другим образом понимаю, какую операцию выполнять. (запихал все операции в массив)
enum Comands {
...
int main(){
	...
	for(int i = 0; i < n; i++){
		...
		int a, b, x;
		//Заменил else if на switch, как Вы попросили 
		switch(res){
			case CONST:
				scanf("%d", &x);
				Push(&s, x);
